...
$user = requireLogin(); if ($user['role'] !== 'admin') { header('Location: /index.php'); exit; }
...
if ($isAdmin) {
    $stmt = $db->query("SELECT COUNT(*) as cnt FROM products WHERE status='active'");
    ...
    $stmt = $db->query("SELECT id, name, price, image_path FROM products WHERE status='active' ORDER BY id DESC LIMIT $lim OFFSET $pOff");
...
$comments = [];
if ($isAdmin) {
    $stmt = $db->query("SELECT user_name, user_avatar, content, image_url, likes, created_at FROM comments WHERE is_deleted=0 ORDER BY created_at DESC LIMIT 8");
    ...
    <div class="container container-wide" style="padding-top:40px;padding-bottom:60px;">
        ...
        <div class="charts-row">
            <div class="chart-container">
                <div class="chart-name">📈 每日瀏覽趨勢（近14天）</div>
                <canvas id="viewsChart" height="200"></canvas>
            </div>
            <div class="chart-container">
                <div class="chart-name">💰 每日營收趨勢（近14天）</div>
        ...
        <div class="charts-row">
            <div class="chart-container">
                <div class="chart-name">📊 廣告平台預算分佈</div>
                <canvas id="platformChart" height="200"></canvas>
            </div>
            <div class="chart-container">
                <div class="chart-name">📋 訂單狀態分佈</div>
        ...
        <div class="chart-container" style="margin-bottom:24px;">
            <div class="chart-name">💬 最新用戶評論（來自 market.com.tw）</div>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;margin-top:12px;">
                ...
                <div style="background:var(--bg-darker);border-radius:8px;padding:12px;">
                    ...
                    <?php if ($comment['image_path']): ?>
                        <img src="<?= htmlspecialchars($comment['image_path']) ?>" style="width:100%;height:80px;object-fit:cover;border-radius:6px;margin-bottom:6px;" loading="lazy" alt="">
                    <?php endif; ?>
        ...
        <div class="chart-container" style="margin-bottom:24px;">
            <div class="chart-name">📱 產品列表 — QR Code 掃碼付款 <span style="font-size:0.8rem;color:var(--text-dim);font-weight:400;margin-left:8px;">共 <?= number_format($totalProductCount) ?> 件 | 第 <?= $productPage ?> / <?= number_format($totalProductPages) ?> 頁</span></div>
        ...
        <!-- Recent Orders (REAL buyer names for admin) -->
        <div class="chart-container">
            <div class="chart-name">🛒 最近訂單（完整買家資訊）</div>
