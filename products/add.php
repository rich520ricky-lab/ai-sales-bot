...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (!$name || !$description || $price <= 0) {
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO products (user_id, seller_id, name, description, short_desc, price, image_path, ai_copy, qr_code_path, category, tags, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')");
            $stmt->execute([$user['id'], $user['id'], $name, $description, $shortDesc, $price, $imagePath, $aiCopy, $qrPath, $category, $tags]);
